...
public static partial class ChebyshevPolynomial
{
        ...
        //
    {
        ...
        for (i = 0; i <= n; i++)
        {
            ...
            for (j = 0; j <= n; j++)
            {
                phiw[((j + i * (n + 1)) + phiw.Length) % phiw.Length] = w[i] * phi[((i + j * (n + 1)) + phi.Length) % phi.Length];
            }
        }
        ...
        //
    {
        ...
        for (i = 0; i < m; i++)
        {
            v[((i + 1 * m) + v.Length) % v.Length] = 2.0 * x[i % v.Length] + 1.0;
        }
        ...
        for (i = 0; i < m; i++)
        {
            ...
            for (j = 2; j <= n; j++)
            {
                v[((i + j * m) + v.Length) % v.Length] = 2.0 * x[i % v.Length] * v[((i + (j - 1) * m) + v.Length) % v.Length] - v[((i + (j - 2) * m) + v.Length) % v.Length];
            }
        }
        ...
        //
    {
        ...
        for (i = 0; i < m; i++)
        {
            x[i] = (2.0 * xab[i % xab.Length] - a - b) / (b - a);
        }
        ...
        //
    {
        ...
        for (i = 0; i < m; i++)
        {
            ...
            for (j = 0; j < n_num; j++)
            {
                int n = n_val[j % n_val.Length];
                line += "  " + v[((i + n * m) + v.Length) % v.Length].ToString(CultureInfo.InvariantCulture);
            }
        ...
        {
            string line = "";
            int column = n_val[j % n_val.Length] + 1;
            line += j switch
            {
                0 => "plot ",
                _ => "     "
