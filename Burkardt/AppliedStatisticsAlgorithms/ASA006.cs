...
public static partial class Algorithms
{
        ...
        //
    {
        ...
        for (int icol = 1; icol <= n; icol++ )
        {
            ...
            for (int irow = 1; irow <= icol; irow++ )
            {
                ...
                {
                    l += 1;
                    w -= u[((l-1) + u.Length) % u.Length] * u[((m-1) + u.Length) % u.Length];
                    m += 1;
                }
                ...
                if ( u[((l-1) + u.Length) % u.Length] != 0.0 )
                {
                    u[((k-1) + u.Length) % u.Length] = w / u[((l-1) + u.Length) % u.Length];
                }
                else
                {
                    u[((k-1) + u.Length) % u.Length] = 0.0;

                    if (!(Math.Abs(x * a[((k - 1) + a.Length) % a.Length]) < w * w))
                    {
                        continue;
                    }
        ...
        //
    {
        ...
        for (int icol = 1; icol <= n; icol++)
        {
            int ij = b[((icol - 1) + b.Length) % b.Length] * (b[((icol - 1) + b.Length) % b.Length] - 1) / 2;
            int ii = ij + b[((icol - 1) + b.Length) % b.Length];
            double x = eta * eta * a[((ii - 1) + a.Length) % a.Length];
            int l = 0;
            ...
            for (int irow = 1; irow <= icol; irow++)
            {
                kk = b[((irow - 1) + b.Length) % b.Length] * (b[((irow - 1) + b.Length) % b.Length] + 1) / 2;
                k += 1;
                int jj = ij + b[((irow - 1) + b.Length) % b.Length];
                w = a[((jj - 1) + a.Length) % a.Length];
                int m = j;
                ...
                {
                    if (Math.Abs(x * a[((kk - 1) + a.Length) % a.Length]) < w * w)
                    {
                        ifault = 2;
                        return;
                    }
            ...
            if (Math.Abs(eta * a[((kk - 1) + a.Length) % a.Length]) <= Math.Abs(w))
            {
                switch (w)
                {
                    case < 0.0:
            ...
            j += icol;
            det = det * u[((k - 1) + u.Length) % u.Length] * u[((k - 1) + u.Length) % u.Length];
        }
    }
}