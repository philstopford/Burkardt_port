        ...
        for (i = 0; i <= n; i++)
        {
            ...
            for (j = 0; j <= n; j++)
            {
                ...
                for (k = 0; k <= n; k++)
                {
                    ...
                    bg[((0 + p * 3) + bg.Length)  % bg.Length] = x;
                    bg[((1 + p * 3) + bg.Length)  % bg.Length] = y;
                    bg[((2 + p * 3) + bg.Length)  % bg.Length] = z;
                    p += 1;

                    switch (i)
                    {
                        case > 0:
                            bg[((0 + p * 3) + bg.Length)  % bg.Length] = 2.0 * c[0] - x;
                            bg[((1 + p * 3) + bg.Length)  % bg.Length] = y;
                            bg[((2 + p * 3) + bg.Length)  % bg.Length] = z;
                            p += 1;
                            break;
                    ...
                    switch (j)
                    {
                        case > 0:
                            bg[((0 + p * 3) + bg.Length)  % bg.Length] = x;
                            bg[((1 + p * 3) + bg.Length)  % bg.Length] = 2.0 * c[1] - y;
                            bg[((2 + p * 3) + bg.Length)  % bg.Length] = z;
                            p += 1;
                            break;
                    }
                    ...
                    switch (k)
                    {
                        case > 0:
                            bg[((0 + p * 3) + bg.Length)  % bg.Length] = x;
                            bg[((1 + p * 3) + bg.Length)  % bg.Length] = y;
                            bg[((2 + p * 3) + bg.Length)  % bg.Length] = 2.0 * c[2] - z;
                            p += 1;
                            break;
                    }
